                                                          ...
                                                          const std::vector<int>& /*fixed_indices*/) const
{
  if (velocity_coeff.size() == 0)
  {
    ...
    pci.cost_infos.push_back(
        createSmoothVelocityTermInfo(start_index, start_index+2, static_cast<int>(pci.kin->numJoints())));
    pci.cost_infos.push_back(
        createSmoothVelocityTermInfo(end_index-2, end_index, static_cast<int>(pci.kin->numJoints())));
  }
  else
  {
    //pci.cost_infos.push_back(createSmoothVelocityTermInfo(start_index, end_index, velocity_coeff));
    pci.cost_infos.push_back(createSmoothVelocityTermInfo(start_index, start_index+3, velocity_coeff));
    pci.cost_infos.push_back(createSmoothVelocityTermInfo(end_index - 3, end_index, velocity_coeff));
  }
